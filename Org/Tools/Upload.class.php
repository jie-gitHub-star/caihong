...
class Upload
{
    private $path = __PUBLIC . '/uploads';          //上传文件保存的路径
    ...
    public function upload($fileField, $mark='file')
    {
        $return = true;
        /* 检查文件路径是滞合法 */
        ...
        /* 将文件上传的信息取出赋给变量 */
        $name =$fileField[$mark]['name'];
        $tmp_name = $fileField[$mark]['tmp_name'];
        $size = $fileField[$mark]['size'];
        $error = $fileField[$mark]['error'];
